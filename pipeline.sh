...
# every prompt-building script reads this; it must match what data/train was built with
...
CKPT=$(ls -d checkpoints/lcpo-exact/checkpoint-* 2>/dev/null \
...
# A baseline captured under different prompt wording is not comparable - the
# base model sees a different instruction. Recapture (~10 min) unless the
# existing base.json was taken with the wording this run trains on.
...
# Credit is thin enough that the box may be stopped mid-pipeline. The headline
# numbers exist as soon as trained.json does, so publish them now rather than
# risk losing them in the checkpoint sweep that follows.
python upload_hf.py --adapter "$CKPT" --repo "$REPO" --results results \
  || echo "[pipeline] WARN: mid-pipeline upload failed, continuing"
...
for STEP in 100 250 500 750 "$FINAL_STEP"; do
  C="checkpoints/lcpo-exact/checkpoint-$STEP"
